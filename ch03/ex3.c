...
int main(void)
{
    ...
    */
    ...
    printf("\n(c) float 1: %f\n(c) float 2: %f\n", c_f1, c_f2);
    /*
    // Note I swapped the order so "%f " is inputted first. This is to
    // illustrate that if the user inputs `1{return}`, the program will hang.
    // For this exact  setup, I provided the first (c) input a value of
    // `1{return}` and the  program hanged, so I then entered "{space}{return}"
    // and the program continued to hang. I then entered "2{return}" and the
    // program completed by printing this way in terminal (`user input`,
    // {invisible character descriptive name}):
    //
